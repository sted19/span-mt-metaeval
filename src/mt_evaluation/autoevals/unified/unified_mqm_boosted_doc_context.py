...
from mt_evaluation.core import Sample, FewShots

logger = logging.getLogger(__name__)


class UnifiedMQMBoostedDocContext(Unified):
    def format_prompts(
        self,
        sample: Sample,
        use_few_shots: bool,
        ...
        if is_reasoning:
            final_instruction = "Execute these steps sequentially. Ensure you complete all steps: error identification, individual error analysis (including reasoning, categorization, and verification), and final annotation generation. Your output outside thinking tags must contain only the final json annotation."
        else:
            final_instruction = "Execute these steps sequentially. Ensure your output shows your reasoning for all steps: error identification, individual error analysis (including reasoning, categorization, and verification), and final annotation generation."

        user_prompt = self.prompt.format_user_prompt(
            src_doc=sample.src_doc,
            src=sample.src,
            tgt=sample.tgt,
...
You will be provided with a source paragraph, its translation, and the source document (the extended context from which the source paragraph was drawn). A paragraph may contain one or more sentences. Your task is to identify all translation errors in the translated paragraph, assigning a category, subcategory, and severity level to each error. Use the source document to resolve ambiguities in the source paragraph and to better evaluate the translation's quality. 
...
## Input Source Document, Source Paragraph, and Translation
The source paragraph and translation to evaluate are provided below:
```
{src_lang} source document: {src_doc}
{src_lang} source paragraph: {src}
